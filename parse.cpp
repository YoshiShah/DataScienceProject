...
shared_ptr<demogData> readCSVLineDemog(std::string theLine) {
  std::stringstream ss(theLine);
    ...
    int FirstNationC = (int)(FirstNation*totalPop2014+.5);
    int AsianC= (int)(Asian*totalPop2014+.5);
    int BlackC = (int)(Black*totalPop2014+.5);
    int LatinxC=(int)(Latinx*totalPop2014+.5);
    int HIPacificIsleC= (int)(HIPacificIsle*totalPop2014+.5);
    int MultiRaceC= (int)(MultiRace*totalPop2014+.5);
    int WhiteC= (int)(White*totalPop2014+.5);
    int WhiteNHC = (int)(WhiteNH *totalPop2014+.5);
    //update as needed this is lab3 version
    return make_shared<demogData>(name, state, popOver65, popUnder18,
            popUnder5,bachelorDegreeUp, highSchoolUp, belowPoverty, totalPop2014, FirstNationC, AsianC, BlackC, LatinxC, HIPacificIsleC, MultiRaceC, WhiteC, WhiteNHC);
}
...
shared_ptr<psData> readCSVLinePolice(std::string theLine) {
    ...
    if(mentalill == "True")
        mi = true;
    else if(mentalill == "False")
        mi = false;
    getFieldNQ(ss);
    string flee = getFieldNQ(ss);
    return make_shared<psData>(state,gender,race,city,flee,name,mi,age);
}
